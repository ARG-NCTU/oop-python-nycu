...
import numpy as np
import time
from lec11_intro_to_ML import variance as g3_variance

# Test variance function correctness
def test_variance_correctness():
    assert g3_variance([1, 2, 3, 4, 5]) == 2.0
    assert g3_variance([1, 1, 1, 1, 1]) == 0.0
    assert g3_variance([1, 2, 3]) == pytest.approx(0.6667, rel=1e-3)
    assert g3_variance([1]) == 0.0
