...
class ListNode:
    ...
    @classmethod    ##following function of this have the first parameter as cls, which is a function that will operate on the this type of class, but the instance object
...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        if head and head.next:
            new_head = self.reverseList(head.next)  ##let new_head point to the last node by recursion
            ##the most important part
            head.next.next = head
            head.next = None
            ##the first lines let a -> b => a - > b -> a
            ##the second let a -> b -> a => a -> None, b -> a
            ##eg: 1 -> 2 -> 3 -> 4 -> 5 => 4 -> 5 -> 4 => 4 -> None, 5 -> 4 ......
        return new_head
