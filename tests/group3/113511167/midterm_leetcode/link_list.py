...
class Solution:
    def removeNodes(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        Examples:
            ...
            True
        """
        def compare_change(head=head):
            if head is None:
                return 

            head.next = compare_change(head.next)
            return head.next if (head.next and head.val < head.next.val) else head
            ##go straight to the last node, and do compare operation from the last one to the first one
