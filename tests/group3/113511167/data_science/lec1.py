...
def greedy(items, max_cost, key_function):
    ...
    items_copy = sorted(items, key = key_function, reverse = True)  ##key is the according to which the items are sorted, reverse = true means that the items are sorted in descending order
    result = []
    ...
    return (result, total_value)    ##result is a list of items that are taken, total_value is the total value of the items taken

def test_greedy(items, constraint, key_function):
    (taken, val) = greedy(items, constraint, key_function) ## equivalent to "taken, val = greedy(items, constraint, key_function)" 
...
def test_greedys(foods, max_units):
    ...
    test_greedy(foods, max_units, Food.get_value)   ##Food.get_value is wrong, Food.get_value(name) should is correct
    print('\nUse greedy by cost to allocate', max_units, 'calories')
    test_greedy(foods, max_units, lambda x: 1/Food.get_cost(x)) ##lambda ...... is a annonymous function
