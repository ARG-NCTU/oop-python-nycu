...
def test_birthday_prob_more():
    ...
    for num_people in [10, 20, 40, 100]:
        est_prob = birthday_prob(num_people, 2, 10000)
        print(f'For {num_people}, est. prob. of a shared birthday is {est_prob:.3f}')
        numerator = math.factorial(366)
        denom = (366 ** num_people) * math.factorial(366 - num_people)
        actual_prob = 1 - numerator / denom
        print(f'Actual prob. for N = {num_people} = {actual_prob:.3f}')
