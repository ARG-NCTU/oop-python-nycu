...
def test_get_ratios():
    ...
    assert get_ratios([0, 0, 0], [0, 2, 4]) == [0.0, 0.0, 0.0]
    assert get_ratios([8, 5, 49], [2, 20, 7]) == [4.0, 0.25, 7.0]
    assert get_ratios([], []) == []

def test_get_stats():
    name_and_grades = [("Alice", [90, 94, 92]), ("Bob", [80, 85, 84]), ("Charlie", [70, 98, 72])]
    assert get_stats(name_and_grades[0]) == ("Alice", [90, 94, 92], 92.0)
    assert get_stats(name_and_grades[1]) == ("Bob", [80, 85, 84], 83.0)
    assert get_stats(name_and_grades[2]) == ("Charlie", [70, 98, 72], 80.0)
