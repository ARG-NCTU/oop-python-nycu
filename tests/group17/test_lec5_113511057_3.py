...
def test_sum_elem_method1():
    assert lec5.sum_elem_method1([10, 20, 30]) == 60
    assert lec5.sum_elem_method1([-10, 10, -20, 20]) == 0
    assert lec5.sum_elem_method1([0, 0, 0]) == 0
    assert lec5.sum_elem_method1([999]) == 999
    assert lec5.sum_elem_method1([]) == 0

def test_sum_elem_method2():
    assert lec5.sum_elem_method2([10, 20, 30]) == 60
    assert lec5.sum_elem_method2([-10, 10, -20, 20]) == 0
    assert lec5.sum_elem_method2([0, 0, 0]) == 0
    assert lec5.sum_elem_method2([999]) == 999
    assert lec5.sum_elem_method2([]) == 0

def new_test_quotient_and_remainder():
    assert lec5.quotient_and_remainder(100, 25) == (4, 0)
    assert lec5.quotient_and_remainder(73, 10) == (7, 3)
    assert lec5.quotient_and_remainder(17, 5) == (3, 2)
    assert lec5.quotient_and_remainder(45, 6) == (7, 3)
