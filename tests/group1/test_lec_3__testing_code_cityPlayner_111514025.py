...
class Graph(Digraph):
    ...
    def add_edge(self, edge):
        ...
        Digraph.add_edge(self, rev) #bi_directions
...
class CityPlanner():
    ...
    def get_shortest_path(self, source, destination, method='dfs'):
        if method == 'dfs':
            ...
            sp = self.shortest_path_bfs(self.g,
                           self.g.get_node(source),
                           self.g.get_node(destination),
                           True)
        if sp is not None:
...
def test_city_planner():
    ...
    shortest_path_bt_dfs = cp.get_shortest_path('Chicago', 'New York', "dfs")
    shortest_path_bt_bfs = cp.get_shortest_path('Chicago', 'New York', "bfs")
    assert shortest_path_bt_dfs == shortest_path_bt_bfs
