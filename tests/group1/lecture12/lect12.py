...
def getData(toScale: bool = False) -> List[Patient]:
    hrList, stElevList, ageList, prevACSList, classList = [], [], [], [], []
    file_path = os.path.join(os.path.dirname(__file__), 'cardiacData.txt')
    with open(file_path, 'r') as cardiacData:
        for line in cardiacData:
            l = line.strip().split(',')
    ...
    points = [
        Patient(f'P{i}', np.array([hrList[i], prevACSList[i], stElevList[i], ageList[i]]), classList[i])
