...
def plot_locs(drunk_kinds, num_steps, num_trials):
    ...
    for d_class in drunk_kinds:
        ...
        pylab.plot(x_vals, y_vals, cur_style,
                   label = d_class.__name__ + ' Mean loc. = <' + str(meanX) + ', ' + str(meanY) + '>')
    pylab.title('Location at End of Walks (' + str(num_steps) + ' steps)')
    pylab.xlabel('Steps East/West of Origin')
    pylab.ylabel('Steps North/South of Origin')
    pylab.legend(loc = 'lower left')
    pylab.show()
def trace_walk(d_class, num_steps):
    f = Field()
    d = d_class()
    f.add_drunk(d, Location(0, 0))
    locs = []
    ...
    for loc in locs:
        x_vals.append(loc.get_x())
        y_vals.append(loc.get_y())
    pylab.plot(x_vals, y_vals, 'b-')
    pylab.title(d_class.__name__ + ' walk')
    pylab.show()
def sim_all_plot(drunk_kinds, num_steps, num_trials):
    style_choice = StyleIterator(('k+', 'r^', 'mo'))
    for d_class in drunk_kinds:
        locs = get_final_locs(num_steps, num_trials, d_class)
        x_vals, y_vals = [], []
